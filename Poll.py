...
class PollSelect(discord.ui.View):
    ...
    async def selectfirst(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        plt.figure(facecolor="#303434", figsize=(4, 3), dpi=1)
        plt.pie(x=x, colors=colors, labels=labels, startangle=90, textprops=textprops)
        plt.savefig(f"tmp/{urllib.parse.quote(self.question.replace(' ', '-').replace('　', '-').replace('?', '_').replace('&', '_'))}_pie.png")
        ...
        image_url = f"http://cdn.kanokiw.com:8443/tmp/{urllib.parse.quote(self.question.replace(' ', '-').replace('　', '-').replace('?', '_').replace('&', '_'))}_pie.png?{random.randint(0, 1000*1000)}"
...
class PollSelect_four(discord.ui.View):
    ...
    async def selectfirst(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        labels = [f"#4 {fourth_rate}%", f"#3 {third_rate}%", f"#2 {second_rate}%", f"#1 {first_rate}%"]
